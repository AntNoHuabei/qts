...
This script intentionally preserves more fields than qwen3-tts-native uses today.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", required=True, help="Upstream Qwen3-TTS Base model id or local path.")
    parser.add_argument("--ref-audio", required=True, help="Reference audio path/URL/base64/path-like accepted by qwen-tts.")
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(
            "qwen-tts is required. Install the upstream package first, for example: pip install -U qwen-tts"
        ) from exc
    ...
    payload = {
        ...
        "model_id": args.model,
        "speaker_encoder_sample_rate_hz": getattr(model.model, "speaker_encoder_sample_rate", None),

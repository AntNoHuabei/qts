...
- `.github/workflows/build-cli.yml` builds accelerated `qwen3-tts-cli` archives for Linux, macOS, and Windows on pull requests and `main` (`vulkan` on Linux/Windows, `metal+coreml` on macOS), and uploads tagged `v*` builds to GitHub Releases
